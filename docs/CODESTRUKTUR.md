...
| 11 | chart.js | `drawDisabledBar` (Helper, auch von lr-balance.js genutzt), `_drawRefElLabel` (Helper, auch von player.js und print-md.js genutzt — zeichnet fettes „Ref.-El."-Label an der Referenzelektroden-Position), `buildCentAxis` (gemeinsame Cent-x-Achsen-Berechnung für `drawChart` und `drawLvChart`: Cent-Position pro Elektrode re 1000 Hz, minimaler Pixelabstand, Ausdünn-Schritt für Cent-Beschriftung), `_attachAxisTooltip` + `_axisTooltipHandler` (Hover-Tooltip „Elektrode N / Hz / ¢" über x-Achsen-Labels; nutzt `cv._axisHits` vom Caller, gemeinsame Tooltip-Div `#axisTooltip`), `drawChart` (Meßergebnisse, x-Achse cent-skaliert), `drawFreqMatchChart`, `_fmcTooltipHandler` |
