...
| 6a | implant-validate.js | Plausibilitätsprüfung User-Eingaben Implantat-Reiter. Aufruf-Schema: `validateImplantTable(side)` wird am Ende von `buildFreqTable` (freq-table.js) aufgerufen. Sammelt Warnungen aus internen `_implCheck…`-Funktionen (Schema `{level: 1|2|3, electrodeIdx?, field?: 'hz'|'thr'|'upper', globalEl?: 'c'|'idr'|'iidr', messageKey, messageParams}`), markiert Eingabefelder per CSS-Klassen `impl-warn-red/orange/yellow` (strengste Stufe gewinnt) und rendert die Liste in `#implValidateList`. Sprachsetzer `_implValidateApplyLang()` wird aus `i18n.js` aufgerufen. BA 133–134 implementiert: Hz-Monotonie (Level 1), Hz-Range (Level 1, Konstante IMPL_VAL_HZ_RANGE pro Hersteller), Hz-Größenordnung (Level 2, Faktor 5 gegen Default). |
