...
| 0a | debug.js | Debug-Panel (Bauanleitung 80/81). Globale API `window.dbg` mit `isActive`/`activate`/`deactivate`/`toggle`/`log`/`set`/`unset`/`test`. Aktivierung über Doppelklick auf `.brand-logo`, URL-Parameter `?debug=1` (oder `?debug=0` zum Deaktivieren) oder programmatisch. Status persistiert in `localStorage` unter `ciSb.debugActive`. Panel als rechte Sidebar (fixed, 360 px); im aktiven Zustand schiebt `body.dbg-active { padding-right: 360px }` den Hauptinhalt nach links. — Seit BA 81: Felder-Speicher getrennt in `_autoFields` (vom Polling befüllt) und `_userFields` (via `dbg.set(...)`); `_allFields()` liefert den Merge (user gewinnt bei Konflikt). Polling-basierter Feldsammler `_collectFields()` läuft per `setInterval(_, 500 ms)`, solange das Panel aktiv ist, und liest globale Variablen (`lang`, `activeSide`, `mfr`, `sideData[…].implant`, `pPlaying`, `pPlaybackMode`, `pOff`, `plEqOn`, `plApplyBalance`, `pCtx`, `pMaplawOn`/`SollC`/`GetIstC`, `pWarpOn`/`Mode`/`Strength`, `sActive`/`Endless`/`OfflineMode`/`CurRec`) sowie DOM-Selektoren (`.tab.active`, `.subtab.active`, `#plVol`). Markdown-Snapshot in `_mdHeader`/`_mdFieldsBlock`/`_mdTestsBlock`/`_mdLogBlock` zerlegt; pro Sektion ein „kopieren"-Knopf kopiert nur diesen Block (inkl. Header). „Alles kopieren"-Button im Header schreibt alle Sektionen. Eigener DOMContentLoaded-Handler. Muß vor `mobile.js` geladen werden, damit andere Module früh `dbg.log`/`dbg.set` rufen können. |
