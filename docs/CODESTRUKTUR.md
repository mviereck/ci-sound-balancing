...
| 13 | tabs-eq.js | `switchTab`, `updateTabLockState`, `updPlSrcButtons`, `updEqToggleBtn`, `updBalApplyBtn`, `updLatApplyBtn`. Sperre umfaßt Top-Level-Tabs **und** Sub-Tabs in Messungen. Globale Variable `_suppressHashPush` (bool) unterdrückt `history.pushState`-Aufrufe während Init und popstate-Restore. `popstate`-Listener am Dateiende reaktiviert Tab/Subtab bei Browser-Navigation (Zurück/Vor). |
