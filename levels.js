...
function buildLvGrid() {
  ...
  g.innerHTML = "";
  // Safety: ensure manualLevels has correct length
  if (!manualLevels || manualLevels.length !== nEl) {
    sideData[activeSide].manualLevels = new Array(nEl).fill(0);
    manualLevels = sideData[activeSide].manualLevels;
  }
  ...
  for (let i = 0; i < nEl; i++) {
    ...
    lbl.addEventListener("click", () => {
      lvFocus = i;
      updLvFocus();
      const inp = document.querySelector(`.lv-num[data-i="${i}"]`);
      if (inp) inp.focus();
    });
    const bw = document.createElement("div");
    bw.className = "lv-bar-wrap";
    bw.dataset.i = String(i);
    const bar = document.createElement("div");
    bar.className = "lv-bar";
    const v = manualLevels[i],
      pct = Math.min((Math.abs(v) / maxRange) * 50, 50);
    if (v >= 0) {
    ...
    num.max = 20;
    num.step = "0.1";
    num.value = manualLevels[i].toFixed(1);
    num.disabled = ex;
    num.dataset.i = String(i);
    num.addEventListener("change", function () {
      const nv = parseFloat(this.value) || 0;
      manualLevels[+this.dataset.i] = Math.max(-20, Math.min(20, nv));
      this.value = manualLevels[+this.dataset.i].toFixed(1);
      lvOnChange();
    });
    ...
    val.className = "lv-val";
    val.id = "lvEff" + i;
    val.dataset.i = String(i);
    const e = eff[i];
    val.textContent = (e >= 0 ? "+" : "") + e.toFixed(1);
    val.style.color = ex
      ? "#999"
      : e > 0.05
        ? "#2563eb"
    ...
    if (i === lvFocus) {
      [lbl, bw, val].forEach((el) => el.classList.add("lv-focus"));
    }
    g.appendChild(lbl);
    g.appendChild(bw);
    g.appendChild(num);
...
function updLvFocus() {
  document.querySelectorAll(".lv-lbl,.lv-bar-wrap,.lv-val").forEach((el) => {
    el.classList.toggle(
      "lv-focus",
      el.dataset.i !== undefined && +el.dataset.i === lvFocus,
    );
  });
}
function lvOnChange() {
...
function updAllBars() {
  const eff = getEffectiveLevels();
  const maxRange = 20;
  document.querySelectorAll(".lv-bar-wrap").forEach((bw) => {
    const i = +bw.dataset.i;
    const v = manualLevels[i];
    const bar = bw.querySelector(".lv-bar");
    const pct = Math.min((Math.abs(v) / maxRange) * 50, 50);
    bar.className = "lv-bar " + (v >= 0 ? "pos" : "neg");
    bar.style.width = pct + "%";
  });
  document.querySelectorAll(".lv-num").forEach((num) => {
    const i = +num.dataset.i;
    if (!isNaN(i)) num.value = manualLevels[i].toFixed(1);
  });
  for (let i = 0; i < nEl; i++) {
    const val = document.getElementById("lvEff" + i);
    if (val) {
      const e = eff[i];
      val.textContent = (e >= 0 ? "+" : "") + e.toFixed(1);
      val.style.color = e > 0.05 ? "#2563eb" : e < -0.05 ? "#dc2626" : "#666";
    }
  }
}
