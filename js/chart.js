...
function drawFRQGraph(cv, rows, cfg) {
  ...
  rows.forEach(function (r) {
    ...
    const imRauschen = Math.abs(r.yCent) <= r.residuumCent;
    const farbe = imRauschen ? "#16a34a" : "#dc2626";
  ...
  arrowRows.forEach(function (a, i) {
    const y = yBase - laneOf[i] * laneH;
    const imRauschen = Math.abs(a.r.yCent) <= a.r.residuumCent;
    const c = imRauschen ? "#16a34a" : "#dc2626";
