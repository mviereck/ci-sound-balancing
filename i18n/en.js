...
Object.assign(L.en, {
    ...
    implValidateIIDRRangeHard: "IIDR ({val} dB) is outside the plausible range ({min}–{max} dB)",
    implValidateIIDRRangeTypical: "IIDR ({val} dB) is outside the typical range ({min}–{max} dB) — default is 40 dB",
    sweep: "▶ Sweep",
    stop: "■ Stop",
    ...
    recommend:
      "Recommendation: <br> - Set the volume so loud that it is almost uncomfortable.<br> - If possible, use a Bluetooth connection for streaming.<br> - First run the “Complete” test procedure, then “Convergence”. Both can be repeated any number of times.<br> - “Complete” is divided into rounds. In each round, every electrode is compared exactly once. First usable estimates are available after about three or four completed rounds; reliable values only after all rounds are finished.<br> - If interrupted, the next start of “Complete” resumes where it stopped.<br> - A marker below the slider shows the value calculated from all measurements so far; the colored area shows the uncertainty of this estimate.<br> - In the Measurement results tab, the quality of the adjustment can be judged by the colors.",
    ...
    pairsRem: "Comparisons remaining",
    exclDuring: "excluded",
    ...
    eeDesc:
      "Exports the correction curve as an EasyEffects preset for PipeWire/PulseAudio.",
    ...
    fmModeSlider:      "Pre-estimate (slider)",
    ...
    fmrClearSliderBtnLabel: "🗑 Delete only pre-estimate (slider)",
    ...
    fmrClearSliderConfirm: "Delete only the slider pre-estimates? The adaptive results will be preserved.",
    ...
    fmExplainSlider:   "The pre-estimate test procedure allows a frequency adjustment per electrode using a slider until the tones sound equally high on both sides. These values are used as a starting point for the more accurate adaptive test procedure, to shorten the test time. After starting the adaptive test procedure, slider adjustment is no longer possible.",
    fmSliderInstruction: "Adjust the slider until both tones sound equally high.",
    fmExplainSliderScience: "The pre-estimate asks you to manually shift the frequency on one side so that the tones sound equally high on both sides. This is a subjective, fast method — its accuracy depends on how confident you are when moving the slider. <br><br>The values do not replace an adaptive procedure but serve as a <strong>starting point</strong>: the adaptive procedure gets significantly shorter test times with a good pre-estimate, because it does not have to start from 0 cents.<br><br>The slider method is not a scientific measurement procedure and is particularly susceptible to psychoacoustic misperceptions.",
    cumulativeDb: "Total",
    ...
    supportIntro2: "So that it stays that way and the tool can continue to grow, it needs regular support. Even 1 or 2 euros per month help noticeably. If 50 users participate, the monthly full requirement is covered. One-time donations are also welcome.",
    ...
    supportTableHeadFull: "Full build",
    ...
    supportFutureTitle: "What full funding makes possible:",
    supportFutureIntro: "<em>Planned, currently slowed down due to lack of funds:</em>",
    supportFuturePlan1: "improved test procedures",
    supportFuturePlan2: "better simulation of changed frequency fitting",
    supportFuturePlan3: "support for additional languages",
    supportFuturePlan4: "larger audio library (more sentences, audiobooks, music)",
    supportFutureConsider: "<em>Under consideration:</em>",
    supportFutureConsider1: "Hearing training area",
    ...
    linksMusicRehabDesc: "Rediscovering the joy of music through the cochlear implant is the real hearing goal for almost every user: music should sound as natural as one remembers it. Those who want to lead their brain back to the music experience of the past will find another tool in the Music Rehab area. Together with the rehabilitation programs of the CI manufacturers, radio and music apps, YouTube music videos (with lyrics) and many other techniques, music can become enjoyable again. Music is more complex than speech and accordingly requires more time and practice — the goal is worth it. Solo musicians from all over the world present their instrument individually, so that the brain can get to know each instrument anew.",

    linksToneGenDesc: "Free, simple and uncomplicated to use.",

    linksHoerwikiDesc: "Wiki on hearing provision, hearing aids, cochlear implants and related topics.",

    linksGutenbergDesc: "Project Gutenberg is an online library with more than 75,000 free eBooks.",
